...
import { useCreateHero, useDeleteHero, useGetAllHero, useUpdateHero } from '@/components/hooks/useHero';
import { Hero, HeroCreate } from '@/components/types/hero';

const ComponentsAgsasHero = () => {
    const [view, setView] = useState<'list' | 'grid'>('list');
    const [addModal, setAddModal] = useState(false);
    const [search, setSearch] = useState('');

    const { data: heroResponse, refetch } = useGetAllHero();
    const createMutation = useCreateHero();
    const updateMutation = useUpdateHero();
    const deleteMutation = useDeleteHero();

    const defaultParams: Hero = {
        id: 0,
        image: '',
        is_active:true,
        createdAt: '',
    ...
    const [params, setParams] = useState<Hero>(defaultParams);
    const [items, setItems] = useState<Hero[]>([]);
    const [filteredItems, setFilteredItems] = useState<Hero[]>([]);

    /* ================= FETCH DATA ================= */
    useEffect(() => {
        if (heroResponse?.data) {
            setItems(heroResponse.data);
            setFilteredItems(heroResponse.data);
        }
    }, [heroResponse]);
   ...
   const changeValue = (
    ...
    const saveHero = () => {
        ...
        if (params.id) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'Do you want to update this hero?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    updateMutation.mutate(params, {
                        onSuccess: () => {
                            ...
                            showMessage('Hero updated successfully');
                        },
                        onError: () => showMessage('Failed to update hero', 'error'),
                    });
                }
            });
        } else {
            const payload: HeroCreate = {
                image: params.image,
            ...
            createMutation.mutate(payload, {
                onSuccess: () => {
                    ...
                    setAddModal(false);
                    setParams(defaultParams);
                    showMessage('Hero added successfully');
                },
            });
    ...
    /* ================= EDIT & DELETE ================= */
    const editHero = (item?: Hero) => {
        setParams(item ? { ...item } : defaultParams);
        setAddModal(true);
    };

    const deleteHero = (item: Hero) => {
        Swal.fire({
            title: 'Are you sure?',
            text: `Do you want to delete "${item.image}"?`,
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                deleteMutation.mutate(item.id, {
                    onSuccess: () => {
                        refetch();
                        showMessage('Hero deleted successfully');
                    },
                    onError: () => showMessage('Failed to delete hero', 'error'),
                });
            }
        });
    };
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex gap-3 items-center">
                        <button className="btn btn-primary" onClick={() => editHero()}>
                            <IconUserPlus className="ltr:mr-2 rtl:ml-2" /> Add Hero
                        </button>
                        <button
                            ...
                            {filteredItems.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td className="!text-center flex justify-center gap-2">
                                        <button className="btn btn-sm btn-outline-primary" onClick={() => editHero(item)}>Edit</button>
                                        <button className="btn btn-sm btn-outline-danger" onClick={() => deleteHero(item)}>Delete</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    ...
                    {filteredItems.map((item) => (
                        <div key={item.id} className="panel p-4 text-center shadow rounded-md">
                            ...
                            <div className="mt-4 flex justify-between gap-2">
                                <button className="btn btn-outline-primary w-1/2" onClick={() => editHero(item)}>Edit</button>
                                <button className="btn btn-outline-danger w-1/2" onClick={() => deleteHero(item)}>Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
...
export default ComponentsAgsasHero;
