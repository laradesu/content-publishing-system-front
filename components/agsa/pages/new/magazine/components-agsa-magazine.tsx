...
import { Magazines, MagazinesCreate, MagazinesResponse } from '@/components/types/news';
import { createMagazines, deleteMagazinesById, getAllMagazines, updateMagazinesById } from '@/components/services/news';

const ComponentsAgsaMagazines = () => {
    const queryClient = useQueryClient();
    const [view, setView] = useState<'list' | 'grid'>('list');
    const [addModal, setAddModal] = useState(false);
    const [search, setSearch] = useState('');

    const defaultParams: Magazines = {
        id: 0,
        title: '',
        file: null,
         is_active:true,
    ...
    const [params, setParams] = useState<Magazines>(defaultParams);
    const [items, setItems] = useState<Magazines[]>([]);
    const [filteredItems, setFilteredItems] = useState<Magazines[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                showMessage('Failed to load magazines', 'error');
            } finally {
                setLoading(false);
            }
        };
    ...
    const fetchMagazine = async () => {
        try {
            ...
            showMessage('Failed to load magazine', 'error');
        }
    };
    ...
    const createMutation = useMutation({
        mutationFn: (payload: MagazinesCreate) => createMagazines(payload),
        onSuccess: () => {
            queryClient.invalidateQueries();
            showMessage('Magazines added successfully');
            setAddModal(false);
            setParams(defaultParams);
            fetchMagazine();
        },
        onError: () => showMessage('Failed to add magazines', 'error'),
    });

    /* ================= UPDATE ================= */
    const updateMutation = useMutation({
        mutationFn: (data: Magazines) =>
            updateMagazinesById(data.id, {
                title: data.title,
                file: data.file,
        ...
        onSuccess: () => {
            queryClient.invalidateQueries();
            showMessage('magazines updated successfully');
            setAddModal(false);
            setParams(defaultParams);
            fetchMagazine();
        },
        onError: () => showMessage('Failed to update magazines', 'error'),
    });

    /* ================= DELETE ================= */
    const deleteMutation = useMutation({
        mutationFn: (id: number) => deleteMagazinesById(id),
        onSuccess: () => {
            queryClient.invalidateQueries();
            showMessage('Magazines deleted successfully');
            fetchMagazine();
        },
        onError: () => showMessage('Failed to delete magazines', 'error'),
    });

    // Save magazines (Add or Update)
    const MagazinesProject = () => {
        ...
        if (params.id) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'Do you want to update this magazines?',
                icon: 'warning',
            ...
            createMutation.mutate(payload, {
                ...
                onError: (error: any) => {
                    console.error('Create magazines error:', error.response?.data || error.message);
                    showMessage('Failed to add magazines', 'error');
                },
            });
        }
    };
    ...
    // Edit magazine
    const editMagazines = (item?: Magazines) => {
        setParams(item ? { ...item } : defaultParams);
        setAddModal(true);
    };

    // Delete magazine
    const deleteMagazines = (item: Magazines) => {
        Swal.fire({
            title: 'Are you sure?',
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <h2 className="text-xl">Magazines</h2>
                <div className="flex w-full flex-col gap-4 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
                    <div className="flex gap-3">
                        <button className="btn btn-primary" onClick={() => editMagazines()}>
                            <IconUserPlus className="ltr:mr-2 rtl:ml-2" /> Add Magazines
                        </button>
                        <button
                                    ...
                                    <td className="!text-center">
                                        <div className="flex items-center justify-center gap-4">
                                            <button
                                                ...
                                                onClick={() => editMagazines(item)}
                                            >
                                                Edit
                                            </button>
                                            <button
                                                className="btn btn-sm btn-outline-danger"
                                                onClick={() => deleteMagazines(item)}
                                            >
                                                Delete
                                            </button>
                                        </div>
                    ...
                    {filteredItems.map((item) => (
                        <div key={item.id} className="panel p-4 text-center shadow rounded-md flex flex-col">{renderFile(item.file)}
                            ...
                            <div className="mt-4 flex justify-between gap-2">
                                <button className="btn btn-outline-primary w-1/2" onClick={() => editMagazines(item)}>Edit</button>
                                <button className="btn btn-outline-danger w-1/2" onClick={() => deleteMagazines(item)}>Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
                            ...
                            >
                                <DialogPanel className="panel w-full max-w-lg rounded-lg border-0 p-0 text-black dark:text-white-dark">
                                    ...
                                    <div className="bg-[#fbfbfb] py-3 text-lg font-medium px-5 dark:bg-[#121c2c]">
                                        {params.id ? 'Edit magazines' : 'Add magazines'}
                                    </div>
                                    <div className="p-5">
                                        <form>
                                            {/* Title */}
...
export default ComponentsAgsaMagazines;
