...
import { useCreateLongBrief, useDeleteLongBrief, useGetAllLongBrief, useUpdateLongBrief } from '@/components/hooks/useDocument';
import { LongBrief, LongBriefCreate } from '@/components/types/document';

const ComponentsAgsasLongBrief = () => {
    const [view, setView] = useState<'list' | 'grid'>('list');
    const [addModal, setAddModal] = useState(false);
    const [search, setSearch] = useState('');

    const { data: longbriefResponse, refetch } = useGetAllLongBrief();
    const createMutation = useCreateLongBrief();
    const updateMutation = useUpdateLongBrief();
    const deleteMutation = useDeleteLongBrief();

    const defaultParams: LongBrief = {
        id: 0,
        title: '',
        file: null,
        content: '',
    ...
    const [params, setParams] = useState<LongBrief>(defaultParams);
    const [items, setItems] = useState<LongBrief[]>([]);
    const [filteredItems, setFilteredItems] = useState<LongBrief[]>([]);

    /* ================= FETCH DATA ================= */
    useEffect(() => {
        if (longbriefResponse?.data) {
            setItems(longbriefResponse.data);
            setFilteredItems(longbriefResponse.data);
        }
    }, [longbriefResponse]);
    ...
    const saveLongBrief = () => {
        ...
        if (params.id) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'Do you want to update this long brief?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    updateMutation.mutate(params, {
                        onSuccess: () => {
                            ...
                            showMessage('Long Brief updated successfully');
                        },
                        onError: () => showMessage('Failed to update long brief', 'error'),
                    });
                }
            });
        } else {
            const payload: LongBriefCreate = {
                title: params.title,
            ...
            createMutation.mutate(payload, {
                onSuccess: () => {
                    ...
                    setAddModal(false);
                    setParams(defaultParams);
                    showMessage('Long Brief added successfully');
                },
            });
    ...
    /* ================= EDIT & DELETE ================= */
    const editLongBrief = (item?: LongBrief) => {
        setParams(item ? { ...item } : defaultParams);
        setAddModal(true);
    };

    const deleteLongBrief = (item: LongBrief) => {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                deleteMutation.mutate(item.id, {
                    onSuccess: () => {
                        refetch();
                        showMessage('Long Brief deleted successfully');
                    },
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-xl">Long Brief</h2>

                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex gap-3 items-center">
                        <button className="btn btn-primary" onClick={() => editLongBrief()}>
                            <IconUserPlus className="ltr:mr-2 rtl:ml-2" /> Add Long Brief
                        </button>
                        <button
                            ...
                            {paginatedData?.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td className="!text-center flex justify-center gap-2">
                                        <button className="btn btn-sm btn-outline-primary" onClick={() => editLongBrief(item)}>Edit</button>
                                        <button className="btn btn-sm btn-outline-danger" onClick={() => deleteLongBrief(item)}>Delete</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    ...
                    {filteredItems.map((item) => (
                        <div key={item.id} className="panel p-4 text-center shadow rounded-md">
                            ...
                            <div className="mt-4 flex justify-between gap-2">
                                <button className="btn btn-outline-primary w-1/2" onClick={() => editLongBrief(item)}>Edit</button>
                                <button className="btn btn-outline-danger w-1/2" onClick={() => deleteLongBrief(item)}>Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
...
export default ComponentsAgsasLongBrief;
