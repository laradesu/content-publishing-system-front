...
import { useCreateCompany, useDeleteCompany, useGetAllCompany, useUpdateCompany } from '@/components/hooks/useCompany';
import { Company, CompanyCreate } from '@/components/types/company';

const ComponentsAgsaCompany = () => {
    const [view, setView] = useState<'list' | 'grid'>('list');
    const [addModal, setAddModal] = useState(false);
    const [search, setSearch] = useState('');

    const { data: companyResponse, refetch } = useGetAllCompany();
    const createMutation = useCreateCompany();
    const updateMutation = useUpdateCompany();
    const deleteMutation = useDeleteCompany();

    const defaultParams: Company = {
        id: 0,
        email: '',
        phone: '',
        is_active: true,
    ...
    const [params, setParams] = useState<Company>(defaultParams);
    ...
    useEffect(() => {
        if (companyResponse?.data) {
            setItems(companyResponse.data);
            setFilteredItems(companyResponse.data);
        }
    }, [companyResponse]);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    updateMutation.mutate(params, {
                        ...
                        onError: () => showMessage('Failed to update company', 'error'),
                    });
                }
            });
        } else {
    ...
    const deleteCompany = (item: Company) => {
        Swal.fire({
            title: 'Are you sure?',
            text: `Do you want to delete "${item.email}"?`,
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                deleteMutation.mutate(item.id, {
                    ...
                    onError: () => showMessage('Failed to delete company', 'error'),
                });
            }
        });
    };
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex gap-3 items-center">
                        <button className="btn btn-primary" onClick={() => editCompany()}>
                            <IconUserPlus className="ltr:mr-2 rtl:ml-2" /> Add Company
                        </button>
                        <button
                            ...
                            {paginatedData?.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td className="!text-center flex justify-center gap-2">
                                        <button className="btn btn-sm btn-outline-primary" onClick={() => editCompany(item)}>Edit</button>
                                        <button className="btn btn-sm btn-outline-danger" onClick={() => deleteCompany(item)}>Delete</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    ...
                    {filteredItems.map((item) => (
                        <div key={item.id} className="panel p-4 text-center shadow rounded-md">
                            ...
                            <div className="mt-4 flex justify-between gap-2">
                                <button className="btn btn-outline-primary w-1/2" onClick={() => editCompany(item)}>Edit</button>
                                <button className="btn btn-outline-danger w-1/2" onClick={() => deleteCompany(item)}>Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <Dialog as="div" className="relative z-50" onClose={() => setAddModal(false)}>
                    ...
                    <div className="fixed inset-0 flex items-center justify-center px-4">
                        <DialogPanel className="panel w-full max-w-lg p-5 relative">
                            ...
                            <h3 className="text-lg mb-4">{params.id ? 'Edit Company' : 'Add Company'}</h3>

                            <input id="email" className="form-input mb-3" placeholder="Email" value={params.email} onChange={changeValue} />
                            <input id="phone" className="form-input mb-3" placeholder="Phone Number" value={params.phone} onChange={changeValue}></input>
                            ...
                            <div className="flex justify-end gap-3">
                                ...
                                <button className="btn btn-primary" onClick={saveCompany}>{params.id ? 'Update' : 'Add'}</button>
                            </div>
                        </DialogPanel>
                    </div>
                </Dialog>
...
export default ComponentsAgsaCompany;
