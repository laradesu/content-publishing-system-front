...
import { Bids, BidsCreate, BidsResponse } from '@/components/types/news';
import { createBids, deleteBidsById, getAllBids, updateBidsById } from '@/components/services/news';

const ComponentsAgsaBids = () => {
    const queryClient = useQueryClient();
    const [view, setView] = useState<'list' | 'grid'>('list');
    const [addModal, setAddModal] = useState(false);
    const [search, setSearch] = useState('');

    const defaultParams: Bids = {
        id: 0,
        title: '',
        file: null,
        is_active:true,
    ...
    const [params, setParams] = useState<Bids>(defaultParams);
    const [items, setItems] = useState<Bids[]>([]);
    const [filteredItems, setFilteredItems] = useState<Bids[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                showMessage('Failed to load bids', 'error');
            } finally {
                setLoading(false);
            }
        };
    ...
    const fetchBid = async () => {
        try {
            ...
            showMessage('Failed to load bid', 'error');
        }
    };
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            showMessage('Bids added successfully');
            setAddModal(false);
            setParams(defaultParams);
            fetchBid();
        },
        onError: () => showMessage('Failed to add bids', 'error'),
    });

    /* ================= UPDATE ================= */
    const updateMutation = useMutation({
        mutationFn: (data: Bids) =>
            updateBidsById(data.id, {
                title: data.title,
                file: data.file,
        ...
        onSuccess: () => {
            queryClient.invalidateQueries();
            showMessage('bids updated successfully');
            setAddModal(false);
            setParams(defaultParams);
            fetchBid();
        },
        onError: () => showMessage('Failed to update bids', 'error'),
    });
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries();
            showMessage('Bids deleted successfully');
            fetchBid();
        },
        onError: () => showMessage('Failed to delete bids', 'error'),
    });

    // Save bids (Add or Update)
    const BidsProject = () => {
        ...
        if (params.id) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'Do you want to update this bids?',
                icon: 'warning',
            ...
            createMutation.mutate(payload, {
                ...
                onError: (error: any) => {
                    console.error('Create bids error:', error.response?.data || error.message);
                    showMessage('Failed to add bids', 'error');
                },
            });
        }
    };
    ...
    // Edit bids
    const editBids = (item?: Bids) => {
        setParams(item ? { ...item } : defaultParams);
        setAddModal(true);
    };

    // Delete bids
    const deleteBids = (item: Bids) => {
        Swal.fire({
            title: 'Are you sure?',
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <div className="flex w-full flex-col gap-4 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
                    <div className="flex gap-3">
                        <button className="btn btn-primary" onClick={() => editBids()}>
                            <IconUserPlus className="ltr:mr-2 rtl:ml-2" /> Add Bids
                        </button>
                        <button
                                    ...
                                    <td className="!text-center">
                                        <div className="flex items-center justify-center gap-4">
                                            <button
                                                ...
                                                onClick={() => editBids(item)}
                                            >
                                                Edit
                                            </button>
                                            <button
                                                className="btn btn-sm btn-outline-danger"
                                                onClick={() => deleteBids(item)}
                                            >
                                                Delete
                                            </button>
                                        </div>
                    ...
                    {filteredItems.map((item) => (
                        <div key={item.id} className="panel p-4 text-center shadow rounded-md flex flex-col">{renderFile(item.file)}
                            ...
                            <div className="mt-4 flex justify-between gap-2">
                                <button className="btn btn-outline-primary w-1/2" onClick={() => editBids(item)}>Edit</button>
                                <button className="btn btn-outline-danger w-1/2" onClick={() => deleteBids(item)}>Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
...
export default ComponentsAgsaBids;
