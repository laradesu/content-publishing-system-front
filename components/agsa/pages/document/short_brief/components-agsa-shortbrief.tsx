...
import { useCreateShortBrief, useDeleteShortBrief, useGetAllShortBrief, useUpdateShortBrief } from '@/components/hooks/useDocument';
import { ShortBrief, ShortBriefCreate } from '@/components/types/document';

const ComponentsAgsasShortBrief = () => {
    const [view, setView] = useState<'list' | 'grid'>('list');
    const [addModal, setAddModal] = useState(false);
    const [search, setSearch] = useState('');

    const { data: shortbriefResponse, refetch } = useGetAllShortBrief();
    const createMutation = useCreateShortBrief();
    const updateMutation = useUpdateShortBrief();
    const deleteMutation = useDeleteShortBrief();

    const defaultParams: ShortBrief = {
        id: 0,
        title: '',
        file: null,
        is_active: true,
    ...
    const [params, setParams] = useState<ShortBrief>(defaultParams);
    const [items, setItems] = useState<ShortBrief[]>([]);
    const [filteredItems, setFilteredItems] = useState<ShortBrief[]>([]);

    /* ================= FETCH DATA ================= */
    useEffect(() => {
        if (shortbriefResponse?.data) {
            setItems(shortbriefResponse.data);
            setFilteredItems(shortbriefResponse.data);
        }
    }, [shortbriefResponse]);
    ...
    const saveShortBrief = () => {
        ...
        if (params.id) {
            Swal.fire({
                title: 'Are you sure?',
                text: 'Do you want to update this short brief?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    updateMutation.mutate(params, {
                        onSuccess: () => {
                            ...
                            showMessage('Short Brief updated successfully');
                        },
                        onError: () => showMessage('Failed to update shoet brief', 'error'),
                    });
                }
            });
        } else {
            const payload: ShortBriefCreate = {
                title: params.title,
            ...
            createMutation.mutate(payload, {
                onSuccess: () => {
                    ...
                    setAddModal(false);
                    setParams(defaultParams);
                    showMessage('Short Brief added successfully');
                },
            });
    ...
    /* ================= EDIT & DELETE ================= */
    const editShortBrief = (item?: ShortBrief) => {
        setParams(item ? { ...item } : defaultParams);
        setAddModal(true);
    };

    const deleteShortBrief = (item: ShortBrief) => {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                deleteMutation.mutate(item.id, {
                    onSuccess: () => {
                        refetch();
                        showMessage('Short Brief deleted successfully');
                    },
                    onError: () => showMessage('Failed to delete Short Brief', 'error'),
                });
            }
        });
    };
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-xl">Short Brief</h2>

                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex gap-3 items-center">
                        <button className="btn btn-primary" onClick={() => editShortBrief()}>
                            <IconUserPlus className="ltr:mr-2 rtl:ml-2" /> Add Short Brief
                        </button>
                        <button
                            ...
                            {paginatedData?.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td className="!text-center flex justify-center gap-2">
                                        <button className="btn btn-sm btn-outline-primary" onClick={() => editShortBrief(item)}>Edit</button>
                                        <button className="btn btn-sm btn-outline-danger" onClick={() => deleteShortBrief(item)}>Delete</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    ...
                    {filteredItems.map((item) => (
                        <div key={item.id} className="panel p-4 text-center shadow rounded-md">
                            ...
                            <div className="text-white-dark break-words whitespace-normal max-h-32 overflow-y-auto">
                                {item.content}<br />
                                {renderFile(item.file)}<br />
                            </div>
                            <div className="mt-2">
                                Active: {item.is_active ? 'Yes' : 'No'}
                            </div>
                            <div className="mt-4 flex justify-between gap-2">
                                <button className="btn btn-outline-primary w-1/2" onClick={() => editShortBrief(item)}>Edit</button>
                                <button className="btn btn-outline-danger w-1/2" onClick={() => deleteShortBrief(item)}>Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
...
export default ComponentsAgsasShortBrief;
