...
import { useCreatePlan, useDeletePlan, useGetAllPlan, useUpdatePlan } from '@/components/hooks/useDocument';
import { Plan, PlanCreate } from '@/components/types/document';

const ComponentsAgsasPlan = () => {
    const [view, setView] = useState<'list' | 'grid'>('list');
    const [addModal, setAddModal] = useState(false);
    const [search, setSearch] = useState('');

    const { data: planResponse, refetch } = useGetAllPlan();
    const createMutation = useCreatePlan();
    const updateMutation = useUpdatePlan();
    const deleteMutation = useDeletePlan();

    const defaultParams: Plan = {
        id: 0,
        title: '',
        file: null,
        content: '',
    ...
    const [params, setParams] = useState<Plan>(defaultParams);
    const [items, setItems] = useState<Plan[]>([]);
    const [filteredItems, setFilteredItems] = useState<Plan[]>([]);

    /* ================= FETCH DATA ================= */
    useEffect(() => {
        if (planResponse?.data) {
            setItems(planResponse.data);
            setFilteredItems(planResponse.data);
        }
    }, [planResponse]);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    updateMutation.mutate(params, {
                        ...
                        onError: () => showMessage('Failed to update plan', 'error'),
                    });
                }
            });
        } else {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                deleteMutation.mutate(item.id, {
                    ...
                    onError: () => showMessage('Failed to delete plan', 'error'),
                });
            }
        });
    };
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="flex gap-3 items-center">
                        <button className="btn btn-primary" onClick={() => editPlan()}>
                            ...
                            {paginatedData?.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td className="!text-center flex justify-center gap-2">
                                        <button className="btn btn-sm btn-outline-primary" onClick={() => editPlan(item)}>Edit</button>
                                        <button className="btn btn-sm btn-outline-danger" onClick={() => deletePlan(item)}>Delete</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                    ...
                    {filteredItems.map((item) => (
                        <div key={item.id} className="panel p-4 text-center shadow rounded-md">
                            ...
                            <div className="mt-4 flex justify-between gap-2">
                                <button className="btn btn-outline-primary w-1/2" onClick={() => editPlan(item)}>Edit</button>
                                <button className="btn btn-outline-danger w-1/2" onClick={() => deletePlan(item)}>Delete</button>
                            </div>
                        </div>
                    ))}
                </div>
...
export default ComponentsAgsasPlan;
